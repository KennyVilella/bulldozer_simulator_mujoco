...
Soil::Soil(const mjModel* m, mjData* d, int instance) {
    ...
    // Determining blade ID
    blade_id = mj_name2id(m, mjOBJ_BODY, "blade");

    // Determining hfield ID of terrain
    terrain_id = mj_name2id(m, mjOBJ_HFIELD, "terrain");

    // Determining hfield ID of blade soil
    blade_soil_id = mj_name2id(m, mjOBJ_HFIELD, "blade soil");
...
void Soil::Compute(const mjModel* m, mjData* d, int instance) {
    ...
    std::vector<float> ori = {
        ...
        static_cast<float>(-d->xquat[4*blade_id+1]),
        static_cast<float>(-d->xquat[4*blade_id+2]),
        static_cast<float>(-d->xquat[4*blade_id+3])};
    ...
    if (soil_update) {
        ...
        for (auto jj = 0; jj < m->hfield_nrow[terrain_id]; jj++) {
            for (auto ii = 0; ii < m->hfield_ncol[terrain_id]; ii++) {
                ...
                // Updating the blade soil hfield if necessary
                if (
